...
`checkpointIntervalMs` (optional config field) caps durable checkpoint writes to
at most one per interval per shard instead of one per batch. Unset = per-batch
(prior behavior). On a crash, up to one interval of already-acked records may be
redelivered, so handlers must be idempotent (the same at-least-once contract, a
wider window). Flushed at shard end and on graceful shutdown.
